  ...
  return (
    <div className="space-y-12 animate-fade-in">
      <div className="text-center space-y-4">
        <h2 className="text-6xl font-black text-slate-900 drop-shadow-sm">4대 핵심 비전</h2>
        <p className="text-slate-500 max-w-2xl mx-auto font-medium">우리원이 추구하는 자립과 상생의 핵심 가치입니다.</p>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {[
          ...
          <div key={i} className={`group bg-white border border-slate-100 p-8 rounded-[2rem] hover:border-${item.color}-300 transition-all duration-500 shadow-[0_10px_30px_-15px_rgba(0,0,0,0.08)] hover:shadow-[0_25px_50px_-15px_rgba(0,0,0,0.15)] hover:-translate-y-2 relative overflow-hidden`}>
            ...
            <h3 className="relative z-10 text-4xl font-bold text-slate-900 mb-2">{item.title}</h3>
            <p className="relative z-10 text-slate-500">{item.desc}</p>
          </div>
        ))}
      </div>
    </div>

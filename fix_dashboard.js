...
export default function DashboardHome({
  ...
  const [featureImg1, setFeatureImg1] = useState("https://via.placeholder.com/600x400?text=Growth+and+Independence");
  const [featureImg2, setFeatureImg2] = useState("https://via.placeholder.com/600x400?text=Sharing+and+Love");
  const [partners, setPartners] = useState<{name: string, img: string, url: string}[]>(defaultPartners);
    ...
    const fetchData = async () => {
      try {
        ...
        if (gallery.length > 0) {
          ...
          setGalleryItems([
            { id: 1, title: "따뜻한 겨울나기 지원", content: "겨울철 난방비 지원...", img: "https://via.placeholder.com/400x300", date: "2024.11.15" },
            { id: 2, title: "청소년 장학금 수여식", content: "장학금 수여식 개최...", img: "https://via.placeholder.com/400x300", date: "2024.10.20" },
            { id: 3, title: "의료비 지원 캠페인", content: "의료 사각지대 해소...", img: "https://via.placeholder.com/400x300", date: "2024.09.05" }
          ]);
        }
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {galleryItems.map((item, idx) => (
                ...
                >
                  ...
                  <div className="p-4 flex flex-col flex-1 bg-stone-50 group-hover:bg-indigo-50/30 transition-colors">
                    <h4 className="text-sm font-bold text-stone-900 mb-1 line-clamp-1">
                      {item.title}
                    </h4>

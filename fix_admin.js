...
export default function AdminDashboard() {
  ...
  // Partners State
  const [partners, setPartners] = useState<{id?: string, name: string, img: string, url: string, order: number}[]>([]);
  ...
  const fetchData = async () => {
    try {
      ...
      if (bannerDoc.exists()) {
        ...
        setFeatureImg1(data.img1 || "https://via.placeholder.com/600x400?text=Growth+and+Independence");
        setFeatureImg2(data.img2 || "https://via.placeholder.com/600x400?text=Sharing+and+Love");
      } else {
        setFeatureImg1("https://via.placeholder.com/600x400?text=Growth+and+Independence");
        setFeatureImg2("https://via.placeholder.com/600x400?text=Sharing+and+Love");
      }
  ...
  const addPartner = () => {
    setPartners([...partners, { name: "", img: "", url: "", order: partners.length }]);
  };
  ...
  if (!user) {
    return (
      <div className="flex flex-col items-center justify-center py-32 space-y-6 animate-fade-in">
        ...
        <h2 className="text-3xl font-black text-stone-900 tracking-tight">관리자 로그인</h2>
        <p className="text-stone-500 text-center max-w-md">
          홈페이지 관리를 위해 로그인이 필요합니다. 허가된 관리자 계정으로 로그인해주세요.
        </p>
        <button
          onClick={handleLogin}
          ...
          <div className="space-y-4">
            {existingPosts.length === 0 ? (
              ...
              existingPosts.map((post) => (
                <div key={post.id} className="flex items-center gap-4 p-4 rounded-xl border border-stone-200 hover:border-stone-300 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className={\`text-[10px] font-bold px-2 py-0.5 rounded-full \${post.type === 'gallery' ? 'bg-emerald-100 text-emerald-700' : 'bg-blue-100 text-blue-700'}\`}>
                        {post.type === 'gallery' ? '갤러리' : '공지사항'}
                      </span>
                      <span className="text-xs text-stone-400">{post.date}</span>
                    </div>
                    <h4 className="font-bold text-stone-900 truncate">{post.title}</h4>
                  </div>
                  <button
        ...
        <div className="bg-white rounded-3xl p-8 border border-stone-100 shadow-sm animate-fade-in">
          ...
          <div className="space-y-6">
            {partners.map((partner, idx) => (
              <div key={idx} className="p-6 rounded-2xl border border-stone-200 bg-stone-50 flex flex-col md:flex-row gap-6 items-start">
                <div className="flex-1 space-y-4 w-full">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <label className="text-xs font-bold text-stone-500">기관명</label>
                      <input
                        type="text"
                        value={partner.name}
                    ...
                    <div className="space-y-2">
                      <label className="text-xs font-bold text-stone-500">연결 링크 URL</label>
                      <input
                        type="text"
                        value={partner.url}
                        onChange={(e) => handlePartnerChange(idx, "url", e.target.value)}
                        placeholder="https://..."
                        className="w-full px-3 py-2 rounded-lg border border-stone-300 focus:ring-2 focus:ring-amber-400 focus:border-transparent"
                      />
                    </div>
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-stone-500">로고 이미지 URL (PNG, SVG, JPG 등)</label>
                    <input
                      type="text"
                      value={partner.img}
                      onChange={(e) => handlePartnerChange(idx, "img", e.target.value)}
                ...
                <div className="w-full md:w-48 flex flex-col items-center gap-3">
                  <div className="w-full h-24 rounded-xl border border-stone-200 bg-white flex items-center justify-center p-2">
                    {partner.img ? (
                      <img src={partner.img} alt={partner.name} className="max-h-full max-w-full object-contain mix-blend-multiply" onError={(e) => (e.currentTarget.src = "https://via.placeholder.com/150x50?text=Error")} />
                    ) : (

...
import { Image as ImageIcon, Check, X, Eye, Calendar, User } from "lucide-react";
    ...
    const fetchGallery = async () => {
      try {
        ...
        if (data.length > 0) {
          ...
          setPosts([
             { id: 1, title: "우리원 봉사단과 함께하는 사랑의 연탄 나눔 및 밑반찬 배달", date: "2024.11.16", author: "관리자", views: 245, img: "https://via.placeholder.com/600x400?text=Gallery+1", content: "추운 겨울을 맞아 지역 사회의 소외된 이웃들에게 따뜻한 온기를 전하고자 사랑의 연탄 나눔과 밑반찬 배달 봉사활동을 진행했습니다." },
             { id: 2, title: "탈북민 자립 공방 '희망을 빚다' 일일 체험 클래스", date: "2024.11.10", author: "관리자", views: 182, img: "https://via.placeholder.com/600x400?text=Gallery+2", content: "자립 공방에서 수공예품 제작 체험 클래스를 성황리에 마쳤습니다." },
             { id: 3, title: "가을맞이 지역주민과 함께하는 바자회 현장 스케치", date: "2024.10.25", author: "관리자", views: 310, img: "https://via.placeholder.com/600x400?text=Gallery+3", content: "바자회 수익금은 전액 소외계층을 위해 쓰일 예정입니다." }
          ]);
        }
      } catch (error) {
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-20">
      <div className="flex flex-col md:flex-row justify-between items-end gap-4 border-b border-stone-200 pb-6">
        <div>
          <h2 className="text-3xl font-black text-stone-900 tracking-tight">활동 갤러리</h2>
          ...
          >
            ...
            >
              ...
              <div className="w-full bg-stone-100 relative">
                {selectedPost.img && (
                   <img src={selectedPost.img} alt={selectedPost.title} className="w-full max-h-[60vh] object-contain" />
                )}
              </div>
              ...
              <div className="p-8 md:p-12">
                <div className="flex items-center gap-4 text-sm font-semibold text-stone-500 mb-4">
                  ...
                  <span className="flex items-center gap-1"><User className="w-4 h-4" /> 관리자</span>
                </div>
                ...
                <div className="prose prose-stone max-w-none prose-lg">
                  <p className="whitespace-pre-wrap leading-relaxed text-stone-700">{selectedPost.content}</p>
                </div>
              </div>
            </motion.div>
          </motion.div>
